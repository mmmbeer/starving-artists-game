...
export const useLobbyState = () => {
  ...
  const createGame = useCallback(async (): Promise<LobbySnapshot> => {
    ...
    setError(null);
    try {
      const payload = await createJsonRequest('/api/lobby/create', {
        method: 'POST',
  ...
  const joinGame = useCallback(async (): Promise<LobbySnapshot> => {
    ...
    setError(null);
    try {
      const payload = await createJsonRequest(`/api/lobby/${gameId}/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ playerId, displayName })
      });
  ...
  const leaveGame = useCallback(async (): Promise<LobbySnapshot | null> => {
    ...
    setError(null);
    try {
      const payload = await createJsonRequest(`/api/lobby/${gameId}/leave`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
